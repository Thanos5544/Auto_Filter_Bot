...
# ---- LIMITS - 407 -> 2nd , 480 -> 3rd , 480 -> 4th ----
PRIMARY_LIMIT = 407
SECONDARY_LIMIT = 480
TERTIARY_LIMIT = 480
...
if MULTIPLE_DB:
    if DATABASE_URI2:
        MEDIA_CLASSES.append(Media2)
    if DATABASE_URI3:
        MEDIA_CLASSES.append(Media3)
    if DATABASE_URI4:
        MEDIA_CLASSES.append(Media4)
...
async def save_file(media):
    ...
    if MULTIPLE_DB:
        try:
            # ---- DUPLICATE SAARE DB ME CHECK ----
            for MC in MEDIA_CLASSES:
                if await MC.find_one({"file_id": file_id}):
                    logger.info(f"[SKIP] '{file_name}' already exists in DB.")
                    return False, 0
...
async def get_search_results(chat_id, query, file_type=None, max_results=None, offset=0, filter=False):
    ...
    try:
        ...
        return [], None, 0
    # ---- CAPTION HAR SEARCH PE SCAN (purane jaisa) ----
    ...
    if MULTIPLE_DB:
        ...
        # ---- DEDUPE - same file do baar nahi dikhegi ----
        merged = {}
        for r in reversed(results):
            for f in r:
                merged[f.file_id] = f
        files_all = list(merged.values())
    else:
        files_all = await Media.find(filter_mongo).sort("$natural", -1).limit(fetch_limit).to_list(length=fetch_limit)

    # ---- RELEVANCE - sahi movie upar ----
    files_all = sort_by_relevance(files_all, query_for_sort)
...
async def get_bad_files(query, file_type=None):
    ...
    results = await asyncio.gather(*[
        MC.find(filter_mongo).sort("$natural", -1).to_list(300) for MC in MEDIA_CLASSES
    ])
    files = []
    for r in reversed(results):
        files += r
...
async def get_file_details(query):
    ...
    results = await asyncio.gather(*[
        MC.find(filter).to_list(length=1) for MC in MEDIA_CLASSES
    ])
    for f in results:
        if f: return f
    return []
